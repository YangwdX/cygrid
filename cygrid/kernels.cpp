 ...
 * 
 */
  if (unlikely(!__pyx_v_params->vector.memview)) {PyErr_SetString(PyExc_AttributeError,"Memoryview is not initialized");__PYX_ERR(0, 178, __pyx_L1_error)}
 ...
 * 
 */
  __pyx_v_x = (__pyx_v_distance * sin(__pyx_v_bearing));

  /* "cygrid/kernels.pyx":209
 *     # kernels that have origin at lower left
 *     x = distance * sin(bearing)
 *     y = -distance * cos(bearing)             # <<<<<<<<<<<<<<
 * 
 *     index_x = <uint32_t> (
 */
  __pyx_v_y = ((-__pyx_v_distance) * cos(__pyx_v_bearing));

  /* "cygrid/kernels.pyx":211
 *     y = -distance * cos(bearing)
 * 
 *     index_x = <uint32_t> (             # <<<<<<<<<<<<<<
 *         # assume, refval_x is always zero! for speed
 ...
 */
  ...
  /* "cygrid/kernels.pyx":228
 *         return 0.
 * 
 *     return params.matrix[index_y, index_x]             # <<<<<<<<<<<<<<
 */
  if (unlikely(!__pyx_v_params->matrix.memview)) {PyErr_SetString(PyExc_AttributeError,"Memoryview is not initialized");__PYX_ERR(0, 228, __pyx_L1_error)}
